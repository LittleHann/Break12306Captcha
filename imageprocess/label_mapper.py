...
def label_converter(file_path, separator='\t'):
    ...
    label = np.array(utils.trim_label(label).resize((60, 60)))
...
def worker(model_path, file_list, output_dir, total_workers, worker_id, debug=False):
    ...
    with tf.Session() as sess:
        ...
        q = deque()
        predictions = np.ndarray(shape=(BATCH_SIZE, NUM_LABELS), dtype=np.float32)
        data = np.ndarray(shape=(BATCH_SIZE, IMAGE_SIZE, IMAGE_SIZE, NUM_CHANNELS), dtype=np.float32)
        for idx, path in enumerate(file_list):
            if hash(path) % total_workers == worker_id:
                try:
                    label = label_converter(path)
                except:
                    traceback.print_exc()
                    continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("model_path", action="store", help="the path to model")
    ...
    if args.debug is not False:
        ...
        worker(args.model_path, file_paths, args.output_dir, args.n, args.debug, debug=True)
    else:
        for i in xrange(args.n):
            worker_list.append(Process(target=worker, args=(args.model_path, file_paths, args.output_dir, args.n, i)))
